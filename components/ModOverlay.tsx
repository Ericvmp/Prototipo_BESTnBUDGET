...
import { Modification, Material } from '../types';
...
interface ModOverlayProps {
  ...
  onNavigateMod?: (mod: Modification) => void;
}
...
const ModOverlay: React.FC<ModOverlayProps> = ({ mod, onClose, onNavigateMaterial, onNavigateMod }) => {
  ...
  const currentIndex = MODS_DATA.findIndex(m => m.id === mod.id);
  const prevMod = currentIndex > 0 ? MODS_DATA[currentIndex - 1] : null;
  const nextMod = currentIndex < MODS_DATA.length - 1 ? MODS_DATA[currentIndex + 1] : null;
  ...
  const modRequirements  = getRequirements(mod.materials);
  const recycleResults   = getRequirements(mod.recycleInfo);
  const salvageResults   = getRequirements(mod.salvageInfo);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col animate-fade-in">
      ...
      {/* Navigation arrows */}
      {prevMod && onNavigateMod && (
        <button
          onClick={() => onNavigateMod(prevMod)}
        ...
        >
          ...
          >
            ...
            <div className="p-6 md:p-8">
                    ...
                    <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                      {mod.description.split(', ').map((stat, i) => {
                        ...
                        return (
                          <span
                            key={i}
                            className={`text-[11px] font-semibold px-2.5 py-1 rounded-lg border ${isNegative ? 'bg-red-500/10 border-red-500/20 text-red-400' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-300'}`}
                          >
                            {stat}
                          </span>
                        );
              ...
              {/* Divider */}
              <div className="h-px w-full mb-8" style={{ background: `linear-gradient(to right, transparent, ${rarity.hex}60, transparent)` }} />
